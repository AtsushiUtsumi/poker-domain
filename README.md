...
| `Bet(amount)` | 現在ベットが0のときのみ。最小額はビッグブラインド (保有チップ全額でのオールインベットは例外) |
| `Raise(amount)` | 現在ベットがある場合のみ。最小レイズは現在ベットの2倍 (保有チップ全額でのオールインレイズは例外) |
...
`Bet`/`Raise` も、指定額が保有チップ全額と一致する場合は最小額未満でも合法な
**オールインベット/レイズ**として成立します。それ以外の中途半端な不足額
(全額でも最小額未満でもない)は `InvalidActionError`/`InsufficientChipsError` になります。
